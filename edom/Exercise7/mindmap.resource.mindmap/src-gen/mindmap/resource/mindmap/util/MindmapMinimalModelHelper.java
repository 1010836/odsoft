 ...
 */
public class MindmapMinimalModelHelper {
	
	private final static mindmap.resource.mindmap.util.MindmapEClassUtil eClassUtil = new mindmap.resource.mindmap.util.MindmapEClassUtil();
			...
			if (feature instanceof org.eclipse.emf.ecore.EReference) {
				...
				if (type instanceof org.eclipse.emf.ecore.EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + mindmap.resource.mindmap.util.MindmapStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof java.util.List<?>) {
							java.util.List<org.eclipse.emf.ecore.EObject> list = mindmap.resource.mindmap.util.MindmapListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + mindmap.resource.mindmap.util.MindmapStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof java.util.List<?>) {
						java.util.List<String> list = mindmap.resource.mindmap.util.MindmapListUtil.castListUnchecked(value);
