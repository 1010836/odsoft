 ...
 */
public class MindmapLayoutUtil {
	...
	public void transferLayoutInformationToModel(org.eclipse.emf.ecore.EObject element) {
		...
		for (java.util.Iterator<mindmap.resource.mindmap.mopp.MindmapLayoutInformation> i = layoutInformationAdapter.getLayoutInformations().iterator(); i.hasNext(); ) {
			mindmap.resource.mindmap.mopp.MindmapLayoutInformation layoutInformation = i.next();
	...
	public void transferLayoutInformationFromModel(org.eclipse.emf.ecore.EObject element) {
		...
		if (layoutReference != null) {
			...
			for (java.util.Iterator<org.eclipse.emf.ecore.EObject> i = list.iterator(); i.hasNext(); ) {
				...
				mindmap.resource.mindmap.mopp.MindmapLayoutInformation layoutInformation = createLayoutInformation(layoutModelElement);
	...
	public org.eclipse.emf.ecore.EObject createLayoutInformationModelElement(mindmap.resource.mindmap.mopp.MindmapLayoutInformation layoutInformation, org.eclipse.emf.ecore.EPackage layoutPackage) {
		...
		mindmap.resource.mindmap.grammar.MindmapSyntaxElement syntaxElement = layoutInformation.getSyntaxElement();
		...
		layoutInformationModelElement.eSet(layoutInformationEClass.getEStructuralFeature(SYNTAX_ELEMENT_ID_EATTRIBUTE_NAME), mindmap.resource.mindmap.grammar.MindmapGrammarInformationProvider.getSyntaxElementID(syntaxElement));
		return layoutInformationModelElement;
	}
	
	public mindmap.resource.mindmap.mopp.MindmapLayoutInformation createLayoutInformation(org.eclipse.emf.ecore.EObject layoutInformationModelElement) {
		Object object = null;
		...
		mindmap.resource.mindmap.grammar.MindmapSyntaxElement syntaxElement = mindmap.resource.mindmap.grammar.MindmapGrammarInformationProvider.getSyntaxElementByID((String) layoutInformationModelElement.eGet(layoutInformationModelElement.eClass().getEStructuralFeature(SYNTAX_ELEMENT_ID_EATTRIBUTE_NAME)));
		if (objectFeature != null) {
			object = layoutInformationModelElement.eGet(objectFeature);
		} else if (syntaxElement instanceof mindmap.resource.mindmap.grammar.MindmapPlaceholder) {
			mindmap.resource.mindmap.grammar.MindmapPlaceholder placeholder = (mindmap.resource.mindmap.grammar.MindmapPlaceholder) syntaxElement;
			mindmap.resource.mindmap.IMindmapTokenResolver tokenResolver = new mindmap.resource.mindmap.mopp.MindmapTokenResolverFactory().createTokenResolver(placeholder.getTokenName());
			mindmap.resource.mindmap.IMindmapTokenResolveResult result = new mindmap.resource.mindmap.mopp.MindmapTokenResolveResult();
			tokenResolver.resolve(visibleTokenText, placeholder.getFeature(), result);
			object = result.getResolvedToken();
		}
		return new mindmap.resource.mindmap.mopp.MindmapLayoutInformation(syntaxElement, object, startOffset, hiddenTokenText, visibleTokenText);
	}
	
	public mindmap.resource.mindmap.mopp.MindmapLayoutInformationAdapter getLayoutInformationAdapter(org.eclipse.emf.ecore.EObject element) {
