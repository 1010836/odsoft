...
public abstract class AbstractMindmapDebuggable {
	
	/**
	 * The list of breakpoints, where each breakpoint is represented by its location
	 * (a string) and the line number
	 */
	private java.util.List<mindmap.resource.mindmap.util.MindmapPair<String, Integer>> lineBreakpoints = new java.util.ArrayList<mindmap.resource.mindmap.util.MindmapPair<String, Integer>>();
	...
	public void startEventSocket(int eventPort) {
		try {
			...
			new mindmap.resource.mindmap.util.MindmapRuntimeUtil().logError("Can't create socket connection while launching.", e);
		}
	}
	...
	public void stopEventSocket() {
		try {
			...
			new mindmap.resource.mindmap.util.MindmapRuntimeUtil().logError("Exception while closing socket.", e);
		}
	}
	
	public void sendEvent(mindmap.resource.mindmap.debug.EMindmapDebugMessageTypes command, boolean sendOnlyInDebugMode, String... arguments) {
		if (isDebugMode() || !sendOnlyInDebugMode) {
			mindmap.resource.mindmap.debug.MindmapDebugMessage message = new mindmap.resource.mindmap.debug.MindmapDebugMessage(command, arguments);
	...
	public void evaluateLineBreakpoint(org.eclipse.emf.common.util.URI uri, int currentLine) {
		if (isDebugMode()) {
			...
			for (int i = 0; i < lineBreakpoints.size(); i++) {
				mindmap.resource.mindmap.util.MindmapPair<String, Integer> breakpointLocationAndLine = lineBreakpoints.get(i);
				...
				if (breakpointLine.intValue() == currentLine && breakpointLocation.equals(location)) {
					// suspending...
					setSuspend(true);
					sendEvent(mindmap.resource.mindmap.debug.EMindmapDebugMessageTypes.SUSPENDED, true, new String[] {"breakpoint", "" + currentLine});
					break;
				}
			}
			waitIfSuspended();
	...
	public void waitIfSuspended() {
		...
		sendEvent(mindmap.resource.mindmap.debug.EMindmapDebugMessageTypes.RESUMED, true);
	}
	
	public void addLineBreakpoint(String location, int line) {
		lineBreakpoints.add(new mindmap.resource.mindmap.util.MindmapPair<String, Integer>(location, new Integer(line)));
	}
	
	public void removeLineBreakpoint(String location, int line) {
		lineBreakpoints.remove(new mindmap.resource.mindmap.util.MindmapPair<String, Integer>(location, new Integer(line)));
	}
	...
	public void terminate() {
		sendEvent(mindmap.resource.mindmap.debug.EMindmapDebugMessageTypes.TERMINATED, false);
		stopEventSocket();
	}
	...
	public void startUpAndWait() {
		...
		if (isDebugMode()) {
			sendEvent(mindmap.resource.mindmap.debug.EMindmapDebugMessageTypes.STARTED, true);
			setSuspend(true);
