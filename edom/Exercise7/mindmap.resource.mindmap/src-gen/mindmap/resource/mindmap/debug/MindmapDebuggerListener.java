 ...
 * code).
 */
public class MindmapDebuggerListener<ResultType, ContextType> implements Runnable {
	...
	private mindmap.resource.mindmap.debug.AbstractMindmapDebuggable debuggable;
	/**
	 ...
	 */
	private java.util.Map<Long, mindmap.resource.mindmap.util.MindmapPair<String, Object>> objectMap = new java.util.LinkedHashMap<Long, mindmap.resource.mindmap.util.MindmapPair<String, Object>>();
	...
	public MindmapDebuggerListener(int requestPort) {
		super();
	...
	private void runDebugger() throws java.io.IOException {
		...
		mindmap.resource.mindmap.debug.MindmapDebugMessage command;
		while (!stop) {
			command = communicationHelper.receive(reader);
			if (command == null) {
				break;
			}
			if (command.hasType(mindmap.resource.mindmap.debug.EMindmapDebugMessageTypes.EXIT)) {
				debuggable.terminate();
				stop = true;
			} else if (command.hasType(mindmap.resource.mindmap.debug.EMindmapDebugMessageTypes.RESUME)) {
				debuggable.resume();
			} else if (command.hasType(mindmap.resource.mindmap.debug.EMindmapDebugMessageTypes.STEP_OVER)) {
				debuggable.stepOver();
			} else if (command.hasType(mindmap.resource.mindmap.debug.EMindmapDebugMessageTypes.STEP_INTO)) {
				debuggable.stepInto();
			} else if (command.hasType(mindmap.resource.mindmap.debug.EMindmapDebugMessageTypes.STEP_RETURN)) {
				debuggable.stepReturn();
			} else if (command.hasType(mindmap.resource.mindmap.debug.EMindmapDebugMessageTypes.ADD_LINE_BREAKPOINT)) {
			...
			} else if (command.hasType(mindmap.resource.mindmap.debug.EMindmapDebugMessageTypes.REMOVE_LINE_BREAKPOINT)) {
				String location = command.getArgument(0);
				int line = Integer.parseInt(command.getArgument(1));
				debuggable.removeLineBreakpoint(location, line);
			} else if (command.hasType(mindmap.resource.mindmap.debug.EMindmapDebugMessageTypes.GET_STACK)) {
				final String[] stack = debuggable.getStack();
				String controlStack = mindmap.resource.mindmap.util.MindmapStringUtil.encode('#', stack);
				mindmap.resource.mindmap.debug.MindmapDebugMessage message = new mindmap.resource.mindmap.debug.MindmapDebugMessage(mindmap.resource.mindmap.debug.EMindmapDebugMessageTypes.GET_STACK_RESPONSE, new String[] {controlStack});
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(mindmap.resource.mindmap.debug.EMindmapDebugMessageTypes.GET_FRAME_VARIABLES)) {
				...
				mindmap.resource.mindmap.debug.MindmapDebugMessage message = new mindmap.resource.mindmap.debug.MindmapDebugMessage(mindmap.resource.mindmap.debug.EMindmapDebugMessageTypes.GET_FRAME_VARIABLES_RESPONSE, topVariableIDs);
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(mindmap.resource.mindmap.debug.EMindmapDebugMessageTypes.GET_VARIABLES)) {
				...
				mindmap.resource.mindmap.debug.MindmapDebugMessage message = new mindmap.resource.mindmap.debug.MindmapDebugMessage(mindmap.resource.mindmap.debug.EMindmapDebugMessageTypes.GET_VARIABLES_RESPONSE, varStrings);
				communicationHelper.sendEvent(message, output);
			} else {
	...
	private String convertToString(long id, Object object) {
		...
		return mindmap.resource.mindmap.util.MindmapStringUtil.convertToString(properties);
	}
	...
	private long getObjectID(String name, Object value) {
		mindmap.resource.mindmap.util.MindmapPair<String, Object> pair = new mindmap.resource.mindmap.util.MindmapPair<String, Object>(name, value);
	...
	public mindmap.resource.mindmap.debug.AbstractMindmapDebuggable getDebuggable() {
		return debuggable;
	}
	
	public void setDebuggable(mindmap.resource.mindmap.debug.AbstractMindmapDebuggable debuggable) {
