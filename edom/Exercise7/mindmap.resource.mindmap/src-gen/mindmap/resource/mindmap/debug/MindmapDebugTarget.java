...
public class MindmapDebugTarget extends mindmap.resource.mindmap.debug.MindmapDebugElement implements org.eclipse.debug.core.model.IDebugTarget, mindmap.resource.mindmap.debug.IMindmapDebugEventListener {
	
	/**
	 * associated system process
	 */
	private mindmap.resource.mindmap.debug.MindmapDebugProcess process;
	...
	private mindmap.resource.mindmap.debug.MindmapDebugThread thread;
	 ...
	 * event listeners
	 */
	private java.util.List<mindmap.resource.mindmap.debug.IMindmapDebugEventListener> eventListeners = new java.util.ArrayList<mindmap.resource.mindmap.debug.IMindmapDebugEventListener>();
	
	private mindmap.resource.mindmap.debug.MindmapDebugProxy debugProxy;
	...
	private class DebugEventDispatcher extends org.eclipse.core.runtime.jobs.Job {
		...
		public DebugEventDispatcher() {
			super(new mindmap.resource.mindmap.mopp.MindmapMetaInformation().getSyntaxName() + " Event Dispatch");
			setSystem(true);
		}
		
		protected org.eclipse.core.runtime.IStatus run(org.eclipse.core.runtime.IProgressMonitor monitor) {
			while (!isTerminated()) {
				mindmap.resource.mindmap.debug.MindmapDebugMessage message = communicationHelper.receive(eventReader);
				if (message != null) {
		...
		private void notifyListeners(mindmap.resource.mindmap.debug.MindmapDebugMessage message) {
			Object[] listeners = eventListeners.toArray();
			for (int i = 0; i < listeners.length; i++) {
				((mindmap.resource.mindmap.debug.IMindmapDebugEventListener) listeners[i]).handleMessage(message);
			}
		}
	}
	
	public MindmapDebugTarget(mindmap.resource.mindmap.debug.MindmapDebugProcess process, org.eclipse.debug.core.ILaunch launch, int requestPort, int eventPort) {
		super(launch.getDebugTarget());
		this.process = process;
		this.launch = launch;
		this.thread = new mindmap.resource.mindmap.debug.MindmapDebugThread(this);
		this.threads = new org.eclipse.debug.core.model.IThread[] {this.thread};
		
		// initialize debug proxy
		try {
			this.debugProxy = new mindmap.resource.mindmap.debug.MindmapDebugProxy(this, requestPort);
	 ...
	 */
	public void addEventListener(mindmap.resource.mindmap.debug.IMindmapDebugEventListener listener) {
	 ...
	 * @param listener event listener
	 */
	public void removeEventListener(mindmap.resource.mindmap.debug.IMindmapDebugEventListener listener) {
		eventListeners.remove(listener);
	}
	
	public String getName() throws org.eclipse.debug.core.DebugException {
		return new mindmap.resource.mindmap.mopp.MindmapMetaInformation().getSyntaxName() + " model";
	}
		...
		if (supportsBreakpoint(breakpoint)) {
			try {
				if ((breakpoint.isEnabled() && getBreakpointManager().isEnabled()) || !breakpoint.isRegistered()) {
					mindmap.resource.mindmap.debug.MindmapLineBreakpoint lineBreakpoint = (mindmap.resource.mindmap.debug.MindmapLineBreakpoint) breakpoint;
	...
	public void handleMessage(mindmap.resource.mindmap.debug.MindmapDebugMessage message) {
		try {
			if (message.hasType(mindmap.resource.mindmap.debug.EMindmapDebugMessageTypes.STARTED)) {
				started();
			} else if (message.hasType(mindmap.resource.mindmap.debug.EMindmapDebugMessageTypes.SUSPENDED)) {
				suspend();
			} else if (message.hasType(mindmap.resource.mindmap.debug.EMindmapDebugMessageTypes.TERMINATED)) {
				terminated();
			} else if (message.hasType(mindmap.resource.mindmap.debug.EMindmapDebugMessageTypes.RESUMED)) {
	...
	public mindmap.resource.mindmap.debug.MindmapDebugProxy getDebugProxy() {
