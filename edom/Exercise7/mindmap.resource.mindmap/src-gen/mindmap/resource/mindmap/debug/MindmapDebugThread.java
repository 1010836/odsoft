...
public class MindmapDebugThread extends mindmap.resource.mindmap.debug.MindmapDebugElement implements org.eclipse.debug.core.model.IThread, mindmap.resource.mindmap.debug.IMindmapDebugEventListener {
	
	private boolean suspended = false;
	private mindmap.resource.mindmap.debug.MindmapDebugTarget debugTarget;
	
	public MindmapDebugThread(mindmap.resource.mindmap.debug.MindmapDebugTarget target) {
		super(target);
	...
	public org.eclipse.debug.core.model.IStackFrame[] getStackFrames() throws org.eclipse.debug.core.DebugException {
		if (isSuspended()) {
			mindmap.resource.mindmap.debug.MindmapDebugMessage stack = this.debugTarget.getDebugProxy().getStack();
			String framesData = stack.getArgument(0);
			if (framesData != null && !"".equals(framesData)) {
				java.util.List<String> frames = mindmap.resource.mindmap.util.MindmapStringUtil.decode(framesData, '#');
				...
				for (int i = 0; i < frames.size(); i++) {
					...
					theFrames[frames.size() - i - 1] = new mindmap.resource.mindmap.debug.MindmapStackFrame(getTarget(), data);
				}
				return theFrames;
			}
		}
	...
	public mindmap.resource.mindmap.debug.MindmapDebugTarget getTarget() {
		return debugTarget;
	}
