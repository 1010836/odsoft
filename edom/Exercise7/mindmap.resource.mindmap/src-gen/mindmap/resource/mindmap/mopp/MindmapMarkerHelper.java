 ...
 * contains <code>mindmap.resource.mindmap.IMindmapTextDiagnostic</code>s it uses
 * the more precise information of this extended diagnostic type.
 */
public class MindmapMarkerHelper {
	
	/**
	 * The extension id of the custom marker type that is used by this text resource.
	 */
	public static final String MARKER_TYPE = mindmap.resource.mindmap.mopp.MindmapPlugin.PLUGIN_ID + ".problem";
	...
	private static class MarkerCommandQueue {
		...
		private java.util.List<mindmap.resource.mindmap.IMindmapCommand<Object>> commands = new java.util.ArrayList<mindmap.resource.mindmap.IMindmapCommand<Object>>();
		
		private MutexRule schedulingRule = new MutexRule();
		
		public void addCommand(mindmap.resource.mindmap.IMindmapCommand<Object> command) {
		...
		public void runCommands() {
			java.util.List<mindmap.resource.mindmap.IMindmapCommand<Object>> commandsToProcess = new java.util.ArrayList<mindmap.resource.mindmap.IMindmapCommand<Object>>();
			synchronized(commands) {
				commandsToProcess.addAll(commands);
				commands.clear();
			}
			for (mindmap.resource.mindmap.IMindmapCommand<Object> command : commandsToProcess) {
	 ...
	 */
	public void mark(org.eclipse.emf.ecore.resource.Resource resource, mindmap.resource.mindmap.IMindmapTextDiagnostic diagnostic) {
	...
	protected void createMarkerFromDiagnostic(final org.eclipse.core.resources.IFile file, final mindmap.resource.mindmap.IMindmapTextDiagnostic diagnostic) {
		final mindmap.resource.mindmap.IMindmapProblem problem = diagnostic.getProblem();
		mindmap.resource.mindmap.MindmapEProblemType problemType = problem.getType();
		final String markerID = getMarkerID(problemType);
		COMMAND_QUEUE.addCommand(new mindmap.resource.mindmap.IMindmapCommand<Object>() {
			public boolean execute(Object context) {
				try {
					...
					if (problem.getSeverity() == mindmap.resource.mindmap.MindmapEProblemSeverity.ERROR) {
					...
					mindmap.resource.mindmap.IMindmapTextDiagnostic textDiagnostic = (mindmap.resource.mindmap.IMindmapTextDiagnostic) diagnostic;
					...
					if (diagnostic instanceof mindmap.resource.mindmap.mopp.MindmapResource.ElementBasedTextDiagnostic) {
						org.eclipse.emf.ecore.EObject element = ((mindmap.resource.mindmap.mopp.MindmapResource.ElementBasedTextDiagnostic) diagnostic).getElement();
					...
					java.util.Collection<mindmap.resource.mindmap.IMindmapQuickFix> quickFixes = textDiagnostic.getProblem().getQuickFixes();
					...
					if (!sourceIDs.isEmpty()) {
						marker.setAttribute(org.eclipse.core.resources.IMarker.SOURCE_ID, mindmap.resource.mindmap.util.MindmapStringUtil.explode(sourceIDs, "|"));
					}
	...
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource) {
		for (mindmap.resource.mindmap.MindmapEProblemType nextType : mindmap.resource.mindmap.MindmapEProblemType.values()) {
	 ...
	 */
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource, mindmap.resource.mindmap.MindmapEProblemType problemType) {
		final org.eclipse.core.resources.IFile file = getFile(resource);
		if (file == null) {
			return;
		}
	...
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource, final org.eclipse.emf.ecore.EObject causingObject) {
		...
		final String markerID = getMarkerID(mindmap.resource.mindmap.MindmapEProblemType.UNKNOWN);
	 ...
	 * given type.
	 */
	public String getMarkerID(mindmap.resource.mindmap.MindmapEProblemType problemType) {
	...
	protected void handleException(org.eclipse.core.runtime.CoreException ce) {
		if (ce.getMessage().matches("Marker.*not found.")) {
			...
			new mindmap.resource.mindmap.util.MindmapRuntimeUtil().logError("Error while removing markers from resource:", ce);
		}
	}
		...
		COMMAND_QUEUE.addCommand(new mindmap.resource.mindmap.IMindmapCommand<Object>() {
			public boolean execute(Object context) {
				try {
					...
					mindmap.resource.mindmap.mopp.MindmapPlugin.logError("Can't create marker.", e);
					return false;
				}
			}
		});
