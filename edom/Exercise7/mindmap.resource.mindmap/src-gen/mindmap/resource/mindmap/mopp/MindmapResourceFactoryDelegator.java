...
public class MindmapResourceFactoryDelegator implements org.eclipse.emf.ecore.resource.Resource.Factory {
	
	protected java.util.Map<String, org.eclipse.emf.ecore.resource.Resource.Factory> factories = null;
	
	public MindmapResourceFactoryDelegator() {
		init();
	}
	...
	protected void init() {
		...
		if (new mindmap.resource.mindmap.util.MindmapRuntimeUtil().isEclipsePlatformAvailable()) {
			new mindmap.resource.mindmap.util.MindmapEclipseProxy().getResourceFactoryExtensions(factories);
		}
		if (factories.get("") == null) {
			factories.put("", new mindmap.resource.mindmap.mopp.MindmapResourceFactory());
		}
	}
