...
/**
 * The MindmapTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * MindmapTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class MindmapTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<mindmap.resource.mindmap.mopp.MindmapTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<mindmap.resource.mindmap.mopp.MindmapTaskItem> foundItems = new java.util.ArrayList<mindmap.resource.mindmap.mopp.MindmapTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new mindmap.resource.mindmap.mopp.MindmapTaskItem(keyword, message, localLine, offset, end));
