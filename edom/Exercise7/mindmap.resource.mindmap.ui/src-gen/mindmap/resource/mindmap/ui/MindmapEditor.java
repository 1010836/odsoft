...
/**
 * A text editor for 'mindmap' models.
 * <p>
 * This editor has id <code>mindmap.resource.mindmap.ui.MindmapEditor</code>
 * The editor's context menu has id
 * <code>mindmap.resource.mindmap.EditorContext</code>.
 * The editor's ruler context menu has id
 * <code>mindmap.resource.mindmap.EditorRuler</code>.
 * </p>
 */
public class MindmapEditor extends org.eclipse.ui.editors.text.TextEditor implements org.eclipse.emf.edit.domain.IEditingDomainProvider, org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener, org.eclipse.emf.common.ui.viewer.IViewerProvider, mindmap.resource.mindmap.IMindmapResourceProvider, mindmap.resource.mindmap.ui.IMindmapBracketHandlerProvider, mindmap.resource.mindmap.ui.IMindmapAnnotationModelProvider {
	
	private mindmap.resource.mindmap.ui.MindmapHighlighting highlighting;
	private org.eclipse.jface.text.source.projection.ProjectionSupport projectionSupport;
	private mindmap.resource.mindmap.ui.MindmapCodeFoldingManager codeFoldingManager;
	private mindmap.resource.mindmap.ui.MindmapBackgroundParsingStrategy bgParsingStrategy = new mindmap.resource.mindmap.ui.MindmapBackgroundParsingStrategy();
	private java.util.Collection<mindmap.resource.mindmap.IMindmapBackgroundParsingListener> bgParsingListeners = new java.util.ArrayList<mindmap.resource.mindmap.IMindmapBackgroundParsingListener>();
	private mindmap.resource.mindmap.ui.MindmapColorManager colorManager = new mindmap.resource.mindmap.ui.MindmapColorManager();
	private mindmap.resource.mindmap.ui.MindmapOutlinePage outlinePage;
	...
	private mindmap.resource.mindmap.ui.MindmapPropertySheetPage propertySheetPage;
	...
	private mindmap.resource.mindmap.ui.IMindmapBracketHandler bracketHandler;
	...
	public MindmapEditor() {
		super();
		setSourceViewerConfiguration(new mindmap.resource.mindmap.ui.MindmapSourceViewerConfiguration(this, this, this, colorManager));
	...
	private final class DocumentListener implements org.eclipse.jface.text.IDocumentListener {
		...
		public void documentChanged(org.eclipse.jface.text.DocumentEvent event) {
			bgParsingStrategy.parse(event, getResource(), MindmapEditor.this);
		}
	}
	...
	private class ModelResourceChangeListener implements org.eclipse.core.resources.IResourceChangeListener {
		public void resourceChanged(org.eclipse.core.resources.IResourceChangeEvent event) {
			...
			try {
					...
					public boolean visit(org.eclipse.core.resources.IResourceDelta delta) {
						...
						if (deltaKind == org.eclipse.core.resources.IResourceDelta.CHANGED && delta.getFlags() != org.eclipse.core.resources.IResourceDelta.MARKERS) {
							...
							if (changedResource != null) {
								...
								mindmap.resource.mindmap.IMindmapTextResource currentResource = getResource();
				...
				mindmap.resource.mindmap.ui.MindmapUIPlugin.logError("Unexpected Error: ", exception);
			}
		}
	}
	...
	public void initializeEditor() {
		...
		setEditorContextMenuId("mindmap.resource.mindmap.EditorContext");
		setRulerContextMenuId("mindmap.resource.mindmap.EditorRuler");
	}
	...
	public void createPartControl(org.eclipse.swt.widgets.Composite parent) {
		...
		highlighting = new mindmap.resource.mindmap.ui.MindmapHighlighting(getResource(), viewer, colorManager, this);
		...
		codeFoldingManager = new mindmap.resource.mindmap.ui.MindmapCodeFoldingManager(viewer, this);
	}
	...
	private void initializeResourceObject(org.eclipse.ui.IEditorInput editorInput) {
		...
		mindmap.resource.mindmap.mopp.MindmapNature.activate(inputFile.getProject());
		...
		mindmap.resource.mindmap.IMindmapTextResource loadedResource = (mindmap.resource.mindmap.IMindmapTextResource) resourceSet.getResource(uri, false);
		if (loadedResource == null) {
			try {
				...
				mindmap.resource.mindmap.IMindmapTextResource currentResource = this.resource;
				...
				if (demandLoadedResource instanceof mindmap.resource.mindmap.IMindmapTextResource) {
					setResource((mindmap.resource.mindmap.IMindmapTextResource) demandLoadedResource);
				} else {
					// the resource was not loaded by an EMFText resource, but some other EMF resource
					mindmap.resource.mindmap.ui.MindmapUIPlugin.showErrorDialog("No EMFText resource.", "The file '" + uri.lastSegment() + "' of type '" + uri.fileExtension() + "' can not be handled by the MindmapEditor.");
					// close this editor because it can not present the resource
					close(false);
				}
			} catch (Exception e) {
				mindmap.resource.mindmap.ui.MindmapUIPlugin.logError("Exception while loading resource in " + this.getClass().getSimpleName() + ".", e);
			}
		} else {
	...
	private void setResource(mindmap.resource.mindmap.IMindmapTextResource resource) {
	...
	private Object getOutlinePage() {
		if (outlinePage == null) {
			outlinePage = new mindmap.resource.mindmap.ui.MindmapOutlinePage(this);
	...
	public org.eclipse.ui.views.properties.IPropertySheetPage getPropertySheetPage() {
		if (propertySheetPage == null) {
			propertySheetPage = new mindmap.resource.mindmap.ui.MindmapPropertySheetPage();
	...
	public void setCaret(org.eclipse.emf.ecore.EObject element, String text) {
		try {
			...
			org.eclipse.jface.text.source.ISourceViewer viewer = getSourceViewer();
			mindmap.resource.mindmap.IMindmapTextResource textResource = (mindmap.resource.mindmap.IMindmapTextResource) element.eResource();
			...
			mindmap.resource.mindmap.IMindmapTextScanner lexer = getResource().getMetaInformation().createLexer();
			try {
				lexer.setText(viewer.getDocument().get(destination, length));
				mindmap.resource.mindmap.IMindmapTextToken token = lexer.getNextToken();
			...
			mindmap.resource.mindmap.ui.MindmapUIPlugin.logError("Exception in setCaret()", e);
		}
	}
	...
	protected org.eclipse.jface.text.source.ISourceViewer createSourceViewer(org.eclipse.swt.widgets.Composite parent, org.eclipse.jface.text.source.IVerticalRuler ruler, int styles) {
		org.eclipse.jface.text.source.ISourceViewer viewer = new org.eclipse.jface.text.source.projection.ProjectionViewer(parent, ruler, getOverviewRuler(), isOverviewRulerVisible(), styles) {
			...
			public void setSelection(org.eclipse.jface.viewers.ISelection selection, boolean reveal) {
				if (!MindmapEditor.this.setSelection(selection, reveal)) {
	...
	public void addBackgroundParsingListener(mindmap.resource.mindmap.IMindmapBackgroundParsingListener listener) {
		bgParsingListeners.add(listener);
	}
	
	public void notifyBackgroundParsingFinished() {
		for (mindmap.resource.mindmap.IMindmapBackgroundParsingListener listener : bgParsingListeners) {
			listener.parsingCompleted(getResource());
		}
	}
	
	public mindmap.resource.mindmap.ui.IMindmapBracketHandler getBracketHandler() {
		return bracketHandler;
	}
	
	public void setBracketHandler(mindmap.resource.mindmap.ui.IMindmapBracketHandler bracketHandler) {
	...
	private boolean setSelection(org.eclipse.jface.viewers.ISelection selection, boolean reveal) {
		if (selection instanceof org.eclipse.jface.viewers.IStructuredSelection) {
			...
			if (object instanceof org.eclipse.emf.ecore.EObject) {
				...
				if (!(resource instanceof mindmap.resource.mindmap.IMindmapTextResource)) {
					return false;
				}
				mindmap.resource.mindmap.IMindmapTextResource textResource = (mindmap.resource.mindmap.IMindmapTextResource) resource;
				mindmap.resource.mindmap.IMindmapLocationMap locationMap = textResource.getLocationMap();
				int destination = locationMap.getCharStart(element);
				if (destination < 0) {
					destination = 0;
