 ...
 */
public class MindmapOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "mindmap.resource.mindmap.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "mindmap.resource.mindmap.ui.occurences.declaration";
	...
	private mindmap.resource.mindmap.ui.MindmapTokenScanner tokenScanner;
	...
	private mindmap.resource.mindmap.IMindmapTextResource textResource;
	 ...
	 */
	public MindmapOccurrence(mindmap.resource.mindmap.IMindmapTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, mindmap.resource.mindmap.ui.MindmapTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new mindmap.resource.mindmap.mopp.MindmapMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(mindmap.resource.mindmap.ui.MindmapBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, mindmap.resource.mindmap.ui.MindmapPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, mindmap.resource.mindmap.ui.MindmapPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, mindmap.resource.mindmap.ui.MindmapPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == mindmap.resource.mindmap.ui.MindmapPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(mindmap.resource.mindmap.ui.MindmapOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(mindmap.resource.mindmap.ui.MindmapOccurrence.DECLARATION_ANNOTATION_ID);
	}
