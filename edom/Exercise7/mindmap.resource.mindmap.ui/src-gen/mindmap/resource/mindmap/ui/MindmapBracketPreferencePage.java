 ...
 * </ul>
 */
public class MindmapBracketPreferencePage extends org.eclipse.jface.preference.PreferencePage implements org.eclipse.ui.IWorkbenchPreferencePage {
	
	private static final String[] ALL_LEFT_BRACKETS = new String[] { "{", "(", "[", "<", "\"", "'", };
	private static final String[] ALL_RIGHT_BRACKETS = new String[] { "}", ")", "]", ">", "\"", "'", };
	
	private String BRACKETS_COLOR = mindmap.resource.mindmap.ui.MindmapPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR;
	...
	private String language = new mindmap.resource.mindmap.mopp.MindmapMetaInformation().getSyntaxName();
	
	private mindmap.resource.mindmap.ui.MindmapBracketSet bracketsTmp;
	
	/**
	 * Creates a preference page for bracket setting.
	 */
	public MindmapBracketPreferencePage() {
		super();
		
		mindmap.resource.mindmap.IMindmapMetaInformation metaInformation = new mindmap.resource.mindmap.mopp.MindmapMetaInformation();
	...
	public void init(org.eclipse.ui.IWorkbench workbench) {
		setPreferenceStore(mindmap.resource.mindmap.ui.MindmapUIPlugin.getDefault().getPreferenceStore());
		setDescription("Define the coloring of matching brackets.");
		
		bracketsTmp = new mindmap.resource.mindmap.ui.MindmapBracketSet(null, null);
		for (String languageID : languageIDs) {
			bracketSetTemp.put(languageID, getPreferenceStore().getString(languageID + mindmap.resource.mindmap.ui.MindmapPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		}
	}
	...
	private void handleMatchingBracketsSelection() {
		...
		enableCheckbox.setSelection(getPreferenceStore().getBoolean(		mindmap.resource.mindmap.ui.MindmapPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		enableClosingInside.setSelection(false);
		matchingBracketsColorButton.setEnabled(getPreferenceStore().getBoolean(		mindmap.resource.mindmap.ui.MindmapPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		...
		initializeLanguage();
		bracketsTmp.setBrackets(getPreferenceStore().getString(language + mindmap.resource.mindmap.ui.MindmapPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		...
		bracketsList.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int index : itemIndices) {
					mindmap.resource.mindmap.IMindmapBracketPair bracketPair = bracketsTmp.getBracketPair(index);
		...
		enableClosingInside.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int idx : itemIndices) {
					mindmap.resource.mindmap.IMindmapBracketPair bracketPair = bracketsTmp.getBracketPair(idx);
	...
	protected void performDefaults() {
		enableCheckbox.setSelection(getPreferenceStore().getDefaultBoolean(mindmap.resource.mindmap.ui.MindmapPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		matchingBracketsColorButton.setEnabled(enableCheckbox.getSelection());
		...
		bracketSetTemp.put(language, getPreferenceStore().getDefaultString(language + mindmap.resource.mindmap.ui.MindmapPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		bracketsTmp.setBrackets(bracketSetTemp.get(language));
		bracketsList.setItems(bracketsTmp.getBracketArray());
		enableClosingInside.setSelection(false);
	}
	...
	private void updateActiveEditor() {
		...
		getPreferenceStore().setValue(mindmap.resource.mindmap.ui.MindmapPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX, enableCheckbox.getSelection());
		getPreferenceStore().setValue(language + mindmap.resource.mindmap.ui.MindmapPreferenceConstants.EDITOR_BRACKETS_SUFFIX, bracketSetTemp.get(language));
		...
		if (editor != null && editor instanceof mindmap.resource.mindmap.ui.MindmapEditor) {
			((mindmap.resource.mindmap.ui.MindmapEditor) editor).invalidateTextRepresentation();
		}
	}
